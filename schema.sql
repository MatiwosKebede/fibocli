...
CREATE TABLE Users (
    ...
    time_preference TEXT CHECK (time_preference IN ('morning', 'afternoon', 'evening', 'balanced')) NOT NULL DEFAULT 'balanced',
...
CREATE TABLE Waves (
    ...
    status TEXT CHECK (status IN ('planned', 'active', 'completed')) NOT NULL DEFAULT 'planned',
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE Schedules (
    ...
    task_type TEXT CHECK (task_type IN ('study', 'leaf_review', 'integration', 'break')) NOT NULL,
...
CREATE INDEX idx_nodes_node_type_parent_id ON Nodes(node_type, parent_id, child_order);
...
-- Populating Fibonacci table (up to n=100)
